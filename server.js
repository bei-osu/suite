...
// ─── Frontend ─────────────────────────────────────────────────────────────────
// NOTE: The HTML is built with a regular function using string concatenation
// so that NO Node.js template literal variables leak into the client-side JS.
// The only place we inject server values is in the stats/header section,
// done safely via explicit string concatenation outside the <script> block.
...
`;
    ...
    // Build the full HTML page — no template literals in the JS block below
    ...
    // Admin (hidden by default, shown via JS when admin logs in)
...
// ── State
var tok      = localStorage.getItem('osu_tok') || null;
var usr      = null;
var cuid     = null;
var selStar  = 0;
var adminFilter = 'all';
...
function doLogin() {
    ...
    fetch('/auth/login')
        ...
        .then(function(d) {
            if (!d.url) throw new Error('No login URL returned — check OSU_CLIENT_ID env var');
            var popup = window.open(d.url, 'osu-auth', 'width=520,height=720,scrollbars=yes');
            if (!popup) {
            ...
            function handler(e) {
                ...
                if (e.data.type === 'osu-auth-success') {
                    ...
                    usr = { userId: e.data.userId, username: e.data.username, avatarUrl: e.data.avatarUrl, isAdmin: e.data.isAdmin || false };
                    renderLoggedIn();
...
function doLogout() {
    if (tok) fetch('/auth/logout', { method: 'POST', headers: { 'Authorization': 'Bearer ' + tok } }).catch(function(){});
    tok = null; usr = null;
...
function renderLoggedIn() {
    ...
    if (usr.avatarUrl) {
        var wrap = document.getElementById('av-in');
        var img  = document.getElementById('av-in-img');
...
function loadDash() {
    ...
    fetch('/admin/reviews', { headers: { 'Authorization': 'Bearer ' + tok } })
        .then(function(r) {
            if (!r.ok) { list.innerHTML = '<div class="reviews-empty" style="padding:24px;">Could not load.</div>'; return null; }
            return r.json();
        })
        .then(function(b) {
            if (!b) return;
            ...
            Object.keys(data).forEach(function(uid) {
                data[uid].forEach(function(rv) {
                    if (String(rv.authorUserId) === String(usr.userId)) mine.push(Object.assign({}, rv, { tuid: uid }));
                });
            });
            if (!mine.length) { list.innerHTML = '<div class="reviews-empty" style="padding:32px;">You have not written any reviews yet.</div>'; return; }
            list.innerHTML = '';
            mine.forEach(function(rv, i) {
                var el = document.createElement('div');
                el.className = 'dash-item';
                el.id = 'dash-' + rv.id;
                el.style.animationDelay = (i * 0.05) + 's';
                var editStars = [1,2,3,4,5].map(function(n) {
                    return '<button class="edit-star" data-s="' + n + '" onclick="setES(\'' + rv.id + '\',' + n + ',false)">\u2605</button>';
                }).join('');
                el.innerHTML =
                    ...
                    '<div class="dash-item-actions">' +
                        '<button class="btn btn-edit" onclick="openEdit(\'' + rv.id + '\',\'' + rv.tuid + '\',' + rv.stars + ',\'' + rv.text.replace(/'/g, "\\'") + '\',false)">Edit</button>' +
                        '<button class="btn btn-danger" onclick="deleteReview(\'' + rv.tuid + '\',\'' + rv.id + '\')">Delete</button>' +
                    '</div>' +
                    '<div class="edit-form" id="ef-' + rv.id + '">' +
                        '<div class="edit-stars" id="est-' + rv.id + '">' + editStars + '</div>' +
                    ...
                    '</div>';
                list.appendChild(el);
                renderES(rv.id, rv.stars, false);
            });
        })
        .catch(function() { list.innerHTML = '<div class="reviews-empty" style="padding:24px;color:#ff6b6b;">Failed to load.</div>'; });
}
...
function openEdit(id, tuid, stars, text, profile) {
    ...
    ES[id] = stars;
    var pfx  = profile ? 'pef' : 'ef';
    var form = document.getElementById(pfx + '-' + id);
    if (!form) return;
    form.classList.add('open');
    var ta = document.getElementById((profile ? 'peta' : 'eta') + '-' + id);
    if (ta) ta.value = text;
    renderES(id, stars, profile);
}
function closeEdit(id, profile) {
    var f = document.getElementById((profile ? 'pef' : 'ef') + '-' + id);
...
function renderES(id, n, profile) {
    var wrap = document.getElementById((profile ? 'pest' : 'est') + '-' + id);
    if (!wrap) return;
    wrap.querySelectorAll('.edit-star').forEach(function(b) {
        b.classList.toggle('on', +b.dataset.s <= n);
    });
}
function saveEdit(id, tuid, profile) {
    var ta  = document.getElementById((profile ? 'peta' : 'eta') + '-' + id);
    var ee  = document.getElementById((profile ? 'pee' : 'ee') + '-' + id);
    if (ee) ee.textContent = '';
    var text  = ta ? ta.value.trim() : '';
    var stars = ES[id] || 0;
    if (!text || !stars) { if (ee) ee.textContent = 'Fill in text and stars'; return; }
    fetch('/reviews/' + tuid + '/' + id, {
        method: 'PUT',
    ...
    .then(function(res) {
        ...
        } else { if (ee) ee.textContent = res.d.error || 'Error'; }
    })
    .catch(function() { if (ee) ee.textContent = 'Network error'; });
}
function deleteReview(tuid, id) {
    if (!confirm('Delete this review?')) return;
    fetch('/reviews/' + tuid + '/' + id, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + tok } })
        .then(function(r) {
            if (r.ok) { notify('Review deleted', 'success'); loadDash(); if (cuid === tuid) loadReviews(tuid); }
            else return r.json().then(function(d) { notify(d.error || 'Error', 'error'); });
        })
        .catch(function() { notify('Network error', 'error'); });
}

// ── Admin
function loadAdminRevs() {
    document.getElementById('admin-list').innerHTML = '<div class="reviews-empty" style="padding:40px;"><span class="spinner"></span></div>';
    fetch('/admin/reviews', { headers: { 'Authorization': 'Bearer ' + tok } })
        .then(function(r) {
            if (!r.ok) { document.getElementById('admin-list').innerHTML = '<div class="reviews-empty">Access denied.</div>'; return null; }
            return r.json();
        })
        .then(function(b) {
            if (!b) return;
            ...
            adminRevs = [];
            Object.keys(data).forEach(function(uid) {
                data[uid].forEach(function(rv) { adminRevs.push(Object.assign({}, rv, { tuid: uid })); });
            });
            adminRevs.sort(function(a, b) { return new Date(b.createdAt) - new Date(a.createdAt); });
            document.getElementById('adm-total').textContent   = adminRevs.length;
            document.getElementById('adm-flagged').textContent = adminRevs.filter(function(r) { return r.flagged; }).length;
            renderAdmin();
        })
        .catch(function() { document.getElementById('admin-list').innerHTML = '<div class="reviews-empty">Failed to load.</div>'; });
}
...
function renderAdmin() {
    ...
    q = q.toLowerCase();
    var filt = adminRevs.filter(function(rv) {
        if (adminFilter === 'flagged' && !rv.flagged) return false;
        if (['1','2','3','4','5'].indexOf(adminFilter) >= 0 && rv.stars !== +adminFilter) return false;
        if (q && rv.text.toLowerCase().indexOf(q) < 0 && rv.authorUsername.toLowerCase().indexOf(q) < 0 && rv.tuid.indexOf(q) < 0) return false;
        return true;
    });
...
function loadReviews(uid) {
    ...
    fetch('/reviews/' + uid)
        .then(function(r) {
            if (!r.ok) { body.innerHTML = '<div class="reviews-empty"><div class="empty-icon">!</div><div>Server error ' + r.status + '</div></div>'; return null; }
            return r.json();
        })
        .then(function(revs) {
            if (!revs) return;
            ...
            body.innerHTML = '';
            revs.slice().reverse().forEach(function(rv, i) {
                var mine = usr && String(usr.userId) === String(rv.authorUserId);
                var el = document.createElement('div');
                el.className = 'review-item';
                el.style.animationDelay = (i * 0.05) + 's';
                var editStars = [1,2,3,4,5].map(function(n) {
                    return '<button class="edit-star" data-s="' + n + '" onclick="setES(\'' + rv.id + '\',' + n + ',true)">\u2605</button>';
                }).join('');
                var mineHtml = '';
                if (mine) {
                    mineHtml =
                        '<div class="review-actions">' +
                            '<button class="btn btn-edit" onclick="openEdit(\'' + rv.id + '\',\'' + uid + '\',' + rv.stars + ',\'' + rv.text.replace(/'/g, "\\'") + '\',true)">Edit</button>' +
                            '<button class="btn btn-danger" onclick="deleteReview(\'' + uid + '\',\'' + rv.id + '\')">Delete</button>' +
                        '</div>' +
                        '<div class="edit-form" id="pef-' + rv.id + '">' +
                            '<div class="edit-stars" id="pest-' + rv.id + '">' + editStars + '</div>' +
                ...
                el.innerHTML =
                    ...
                    mineHtml;
                body.appendChild(el);
                if (mine) renderES(rv.id, rv.stars, true);
            });
        })
        .catch(function() { body.innerHTML = '<div class="reviews-empty"><div class="empty-icon">!</div><div>Failed to load.</div></div>'; });
}
...
function setStar(n) {
    selStar = n;
    document.querySelectorAll('.write-star').forEach(function(b) { b.classList.toggle('on', +b.dataset.s <= n); });
    validateSubmit();
}
document.getElementById('write-stars').addEventListener('mouseover', function(e) {
    var b = e.target.closest('.write-star');
    if (!b) return;
    document.querySelectorAll('.write-star').forEach(function(x) { x.classList.toggle('on', +x.dataset.s <= +b.dataset.s); });
});
document.getElementById('write-stars').addEventListener('mouseleave', function() {
    document.querySelectorAll('.write-star').forEach(function(b) { b.classList.toggle('on', +b.dataset.s <= selStar); });
});
function onTAInput() {
...
function submitReview() {
    if (!tok || !cuid) return;
    var text  = document.getElementById('write-ta').value.trim();
    var errEl = document.getElementById('write-err');
    ...
    fetch('/reviews', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + tok },
        body: JSON.stringify({ targetUserId: cuid, stars: selStar, text: text })
    })
    ...
    .then(function(res) {
        if (res.ok) {
            ...
            selStar = 0;
            document.querySelectorAll('.write-star').forEach(function(b) { b.classList.remove('on'); });
            ...
            errEl.textContent = res.d.error || 'Server error';
        }
    })
    .catch(function() { errEl.textContent = 'Could not reach server'; })
    .finally(function() { btn.innerHTML = 'Submit review'; validateSubmit(); });
}
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (method === 'GET' && pathname === '/health') {
            ok(res, { status: 'healthy', uptime: process.uptime(), memory: process.memoryUsage(), version: '2.6.0' }); return;
        }
